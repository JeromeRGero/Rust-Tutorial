...
mod pizza_order {
    ...
    pub mod help_customer {
        ...
        pub fn take_order(meal: Meal) {
            seat_at_table();
            if meal.is_pizzatime() {
                let cust_piz = match meal {
                    Meal::Lunch => Ok(super::Pizza::lunch("veggies")),
                    Meal::Dinner => Ok(super::Pizza::dinner(true, "pepper", 33.3)),
                    Meal::Breakfast => Err("System Error: Contact Admin - \"Breakfast isn't a time for pizza...\"")
                };
                serve_customer(cust_piz.unwrap());
            }
        }
        fn serve_customer(cust_piz: super::Pizza) {
            println!(
                ...
                cust_piz.size_in_oz, 
                cust_piz.dough, 
                if cust_piz.cheese {String::from("")} else {String::from("out")}, 
                if cust_piz.topping.is_empty() {String::from("no")} else {cust_piz.topping});
        }
    }
}
